...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _save() async {
    ...
    await p.setStringList(
      'reports',
      reports.map((x) => jsonEncode(x.toJson())).toList(),
    );
          ...
          builder: (context, setD) => Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                value: activity,
                decoration:
                    const InputDecoration(labelText: 'نوع فعالیت'),
                items: activities
                    .map(
                      (x) => DropdownMenuItem(
                        value: x,
              ...
              TextField(
                ...
                maxLines: 3,
                decoration:
                    const InputDecoration(labelText: 'شرح فعالیت'),
              ),
            ],
          ),
        ),
  ...
  Future<void> exportReports() async {
    ...
    final file = File(
      '${dir.path}/report_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.json',
    );
  ...
  Future<void> importReports() async {
    ...
    final fresh =
        incoming.where((x) => !ids.contains(x.id)).toList();
  ...
  Future<void> exportManagerCsv() async {
    ...
    final file = File(
      '${dir.path}/manager_report_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.csv',
    );
  ...
  Future<void> showMonthlyReport() async {
    ...
    final months = <String>{
      ...reports.map((r) => r.date.substring(0, 7)),
      selectedMonth,
    }.toList()
      ..sort();
    ...
    await showDialog(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setD) {
          ...
          final total =
              filtered.fold<int>(0, (a, r) => a + r.minutes);
          ...
          for (final r in filtered) {
            byActivity[r.activity] =
                (byActivity[r.activity] ?? 0) + r.minutes;

            byExpert[r.expert] =
                (byExpert[r.expert] ?? 0) + r.minutes;
          }
                ...
                child: Column(
                  ...
                  children: [
                    DropdownButtonFormField<String>(
                      value: selectedMonth,
                      decoration:
                          const InputDecoration(labelText: 'ماه'),
                      items: months
                          .map(
                            (m) => DropdownMenuItem(
                              value: m,
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'کارشناس'),
                      items: experts
                          .map(
                            (e) => DropdownMenuItem(
                              value: e,
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    ...
                    subtitle:
                        Text(DateFormat('yyyy-MM-dd').format(from)),
                    trailing:
                  ...
                  ListTile(
                    ...
                    subtitle:
                        Text(DateFormat('yyyy-MM-dd').format(to)),
                    trailing:
                        const Icon(Icons.calendar_month),
                    onTap: () async {
                      final d = await showDatePicker(
...
class ManagerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final minutes =
        reports.fold<int>(0, (s, r) => s + r.minutes);
        ...
        ...reports.reversed.map(
          (r) => Card(
            child: ListTile(
              title: Text(r.activity),
              subtitle: Text(
                '${r.expert} • ${r.date} • ${r.minutes} دقیقه',
              ),
            ),
          ),
        ),
